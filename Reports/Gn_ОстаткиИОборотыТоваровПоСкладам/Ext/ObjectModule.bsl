...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если ОтборСклад <> Неопределено 
		И ОтборСклад.ПравоеЗначение = Тип("СписокЗначений") Тогда
		
		Для Инд = 1 По ОтборСклад.ПравоеЗначение.Количество()  Цикл
			
			ЭлементСписка = ОтборСклад [Инд - 1];
			НоваяСтрока = ТаблицаПорядок.Добавить();
			НоваяСтрока.Ключ = ЭлементСписка.Склад;
	...
	ПолучитьДанныеНаОснованииСКД(СхемаКомпоновкиДанных, НастройкиКомпоновщика, ДокументРезультат, ,ДанныеРасшифровки , , ВнешниеНаборыДанных);
